-- Allow editing/deleting any manual stock-ledger row (not just the latest),
-- by cascading the resulting balance delta to every later transaction for
-- that item, atomically. Receipts and test_usage remain locked here since
-- they're tied to purchase-order receiving / result entry, not free-form.
...
begin
  if p_transaction_type not in (
    'opening_balance','positive_adjustment','negative_adjustment','wastage','expiry','stock_count_correction'
  ) then
