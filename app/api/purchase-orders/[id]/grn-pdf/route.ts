...
import { GrnDocument } from "@/lib/pdf/grn-document";
...
export async function GET(
  ...
  const buffer = await renderPdf(createElement(GrnDocument, { po, settings }));

  return new NextResponse(buffer as BodyInit, {
    headers: {
      "Content-Type": "application/pdf",
      "Content-Disposition": pdfContentDisposition(request, `GRN-${po.po_number}.pdf`),
    },
  });
}
