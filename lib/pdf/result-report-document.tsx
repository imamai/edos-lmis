...
import type { ReleasedResultData } from "@/lib/interop/data";
...
export function ResultReportDocument({ data, settings }: { data: ReleasedResultData; settings: TenantSettings }) {
  return (
    <Document>
      <Page size="A4" style={styles.page}>
        <DocumentHeader settings={settings} title="Laboratory Report" meta={data.testCode} />
        ...
        <View style={styles.section}>
          <View style={{ flexDirection: "row", flexWrap: "wrap" }}>
            ...
            <Text style={{ width: "50%", marginBottom: 3 }}>
              Released: {data.releasedAt ? new Date(data.releasedAt).toLocaleString() : "-"}
            </Text>
          </View>
        </View>

        <View style={styles.section}>
          <Text style={styles.sectionTitle}>{data.testName}</Text>
        ...
        <InfoBox tone="blue" title="Interpretation">
          <Text style={styles.infoBoxText}>{data.interpretation}</Text>
        </InfoBox>
